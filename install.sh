...
alias ark="$HOME/core-opinion/packages/core/bin/run"
echo 'alias opinion="$HOME/core-opinion/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-opinion"
git clone "https://github.com/yonseiyes/opinion-core" "$HOME/core-opinion" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/yonseiyes/opinion-core'"

    exit 1
fi

cd "$HOME/core-opinion"
...
rm -rf "$HOME/.config/@opinion"
rm -rf "$HOME/.config/@opinion"
rm -rf "$HOME/.config/opinion-core"
